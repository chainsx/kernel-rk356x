		...
		int len, uint8_t *data, uint8_t bank)
{
	...
	uint8_t *w_data = (u_int8_t*)kzalloc(data_len * sizeof(uint8_t), GFP_KERNEL);
	uint8_t tmp_addr[4] = {0};
	uint8_t tmp_data[4] = {0};
	bool *chk_data = kzalloc(data_len * sizeof(bool), GFP_KERNEL);
	bool result = false;
	uint32_t chk_idx = 0;
	int i = 0;

	if(w_data == NULL || chk_data == NULL){
	...
	result = (g_core_fp.fp_register_write(tmp_addr, data_len, w_data, 0)== NO_ERR);
	kfree(w_data);
	kfree(chk_data);
